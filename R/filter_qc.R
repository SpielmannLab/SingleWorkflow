...
Usage: filter_qc.R --scobject=<file> --specie=<value> --infolder=<folder> --outfolder=<folder> --minfeat=<value> --maxfeat=<value> --mincellfeat=<value> --maxcellfeat=<value> --mincountcell=<value> --maxcountcell=<value> --mingenecell=<value> --maxgenecell=<value> --pctmt=<value> --pctrb=<value> --mt=<logical> --rb=<logical>
...
features <- data.frame(fData(cds)) %>%
	dplyr::filter(as.numeric(feat_count_depth) > minfeat) %>%
	dplyr::filter(as.numeric(feat_count_depth) < maxfeat) %>%
	dplyr::filter(as.numeric(feat_cell_depth) > mincellfeat) %>%
	dplyr::filter(as.numeric(feat_cell_depth) < maxcellfeat) %>%
	dplyr::select(id) %>%
	unlist %>%
	as.character

cells <- data.frame(pData(cds)) %>%
	dplyr::filter(as.numeric(cell_depth_counts) > mincountcell) %>%
	dplyr::filter(as.numeric(cell_depth_counts) < maxcountcell) %>%
	dplyr::filter(as.numeric(cell_depth_genes_detected) > mingenecell) %>%
	dplyr::filter(as.numeric(cell_depth_genes_detected) < maxgenecell) %>%
	dplyr::filter(as.numeric(pct_mt_counts) < pctmt) %>%
	dplyr::filter(as.numeric(pct_rb_counts) < pctrb) %>%
